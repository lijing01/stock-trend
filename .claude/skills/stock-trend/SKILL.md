...
allowed-tools:
  - Read
  - Write
  # 代码解析：股票/ETF名称→ts_code。用户输入名称或代码时触发
  - Bash(python3 .claude/skills/stock-trend/scripts/core/resolve_code.py *)
  # 数据管线：一键获取K线/资金/基本面/宏观。/stock-trend Step 2A 触发
  - Bash(python3 .claude/skills/stock-trend/scripts/pipeline/runner.py *)
  # 综合评分：融合技术面+四维评分。/stock-trend Step 3 触发
  - Bash(python3 .claude/skills/stock-trend/scripts/analysis/scores.py *)
  # K线获取(Tushare)：Tushare数据源。管线内部调用，非直接触发
  - Bash(python3 .claude/skills/stock-trend/scripts/fetchers/kline.py *)
  # K线获取(东方财富)：免费降级数据源。Tushare不可用时管线自动降级
  - Bash(python3 .claude/skills/stock-trend/scripts/fetchers/kline_eastmoney.py *)
  # ETF扫描：扫描精选ETF池输出排名。trigger: /etf-scan
  - Bash(python3 .claude/skills/stock-trend/scripts/scans/etf_scanner.py *)
  # 技术分析：MA/MACD/RSI/KDJ/布林带等。管线内部调用
  - Bash(python3 .claude/skills/stock-trend/scripts/analysis/technical.py *)
  # ETF专项数据：净值/IOPV/折溢价/规模。管线内部调用(仅ETF标的)
  - Bash(python3 .claude/skills/stock-trend/scripts/fetchers/etf_data.py *)
  # 指数期货：基差/持仓量/成交量信号。管线内部调用(仅ETF标的)
  - Bash(python3 .claude/skills/stock-trend/scripts/fetchers/futures_data.py *)
  # 资金流向：主力/北向/融资融券/龙虎榜。管线内部调用
  - Bash(python3 .claude/skills/stock-trend/scripts/fetchers/capital_flow.py *)
  # 基本面：PE/PB/ROE/营收利润增速。管线内部调用(非ETF)
  - Bash(python3 .claude/skills/stock-trend/scripts/fetchers/fundamental.py *)
  # 宏观快照：汇率/利率/PMI/CPI/M2。管线内部调用
  - Bash(python3 .claude/skills/stock-trend/scripts/fetchers/macro_snapshot.py *)
  # K线图生成：从kline.json生成HTML图表。管线完成后可选触发
  - Bash(python3 .claude/skills/stock-trend/scripts/reporting/chart.py *)
  # 报告生成：从管线+评分生成MD+HTML报告。/stock-trend Step 4 触发
  - Bash(python3 .claude/skills/stock-trend/scripts/reporting/report.py *)
  # 持仓管理：增删改查/盈亏/凯利分析/预警。trigger: /portfolio
  - Bash(python3 .claude/skills/stock-trend/scripts/portfolio/manager.py *)
  # 回测引擎：验证ETF评分模型预测力。trigger: /etf-backtest
  - Bash(python3 .claude/skills/stock-trend/scripts/backtesting/engine.py *)
  # 龙头中军扫描：板块热度+成分股筛选+深度分析。trigger: /longtou
  - Bash(python3 .claude/skills/stock-trend/scripts/scans/market_leader.py *)
  # 板块数据：获取板块排行/成分股。管线内部调用(/longtou + /market-theme)
  - Bash(python3 .claude/skills/stock-trend/scripts/fetchers/sector_data.py *)
  # 市场主线分析：板块持续性评分+趋势强度。trigger: /market-theme
  - Bash(python3 .claude/skills/stock-trend/scripts/analysis/market_theme.py *)
  # 板块K线：获取BK指数历史K线。管线内部调用(/market-theme)
  - Bash(python3 .claude/skills/stock-trend/scripts/fetchers/sector_kline.py *)
  - WebSearch
  - WebFetch
