...
> **分支选择**：根据触发命令执行对应流程。
> - 触发 `/etf-scan` → 仅执行下方的「/etf-scan 流程」，**跳过** `/stock-trend` 的所有步骤
...
## /portfolio [list|add|remove|update|status|alerts] [options]

> 管理 ETF 持仓，追踪浮动盈亏、止损预警、与 etf-scan 排名对比。
