---
name: stock-trend
description: 分析 A股、港股和 ETF 的中线趋势并生成结构化报告；也用于今日推荐、观察列表、每日复盘及候选股。用户提到股票或 ETF 趋势、代码分析、观察列表、选股或复盘时使用。
---

# 股票趋势判断

使用 `$stock-trend` 显式调用；也可直接用自然语言说明标的或工作流。`/stock-trend` 是工作流路由标识，不要求 Codex 提供同名 slash command。
