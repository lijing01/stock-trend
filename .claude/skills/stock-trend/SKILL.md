...
```
📊 ETF 速评分回测报告    {timestamp}

▸ 回测区间: {lookback_days}个交易日
▸ 采样日期: {total_dates}个
▸ 测试ETF: {total_etfs_tested}只
▸ 数据限制: capital_flow/shares_trend/iopv 无历史数据，仅验证 momentum+volume

┌─ IC (信息系数) ──────────────────────────────────────────────┐
│ 窗口       IC均值   标准差    t值   正向比率   说明             │
│ ───────────────────────────────────────────────────────────── │
│ 5日       0.071    0.13    2.72   68%        ★ 有预测力       │
│ 10日      0.058    0.14    2.01   62%        ★ 有预测力       │
│ 20日      0.042    0.15    1.35   58%                        │
└──────────────────────────────────────────────────────────────┘

命中率: Top10 在5日窗口 62%, 10日 58%, 20日 55%
Top vs Bottom 收益差(10日): 3.2%
Top10 平均收益: 5日 +1.2%, 10日 +1.8%, 20日 +2.5%
```

3. **解读说明**
- IC > 0.05 且 5% 显著: 该维度有预测力
- 命中率 > 55%: 模型选股优于随机
- Top vs Bottom 差 > 2%: 选股有经济意义
- 注意：回测仅覆盖 momentum + volume 维度（资金流/份额/IOPV 无历史数据），实际效果可能优于回测结果
...
**龙头**
- 北方华创(002371) 涨跌幅:10.0% ↑看多 ★★★
  technical:1.5 capital_flow:1.0 fundamental:0.8 sentiment:1.0 macro:0.5
  止损:285.0  目标:310.0/335.0
- 中芯国际(688981) 涨跌幅:6.5% ↑偏多 ★★☆
  ...

**中军**
- 韦尔股份(603501) 市值:2850亿 PE:32.5 ↑偏多 ★★☆
  止损:95.0
- 紫光国微(002049) 市值:1200亿 PE:45.0 ↗偏多 ★☆☆
```
