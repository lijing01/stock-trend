...
def _run_script(script, arguments):
    ...
    process = subprocess.run(
        ...
        stdout=subprocess.PIPE, text=True, check=True, timeout=1200)
...
def run_today(candidate_args=None, *, now=None, state_root=DEFAULT_STATE_ROOT, dry_run=False):
    ...
    if notice:
        ...
        notice["monitor_fallback"] = workflow["monitor"].get("fallback")
        output["notifications"].append(notice)
    if workflow["monitor"].get("status") == "fallback_required":
...
def render_summary(result):
    ...
    labels = {"completed": "已完成", "partial": "部分完成", "dry_run": "仅预览",
...
def main(argv=None):
    ...
    result = run_today(candidate_args, dry_run=args.dry_run)
