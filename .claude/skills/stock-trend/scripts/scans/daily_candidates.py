...
def pick_hot_sectors(top_n=None, min_hot=45, min_stocks=10, regime=None,
    ...
    try:
        ...
        LOGGER.debug("Sector ranking failed: %s", classify_failure(exc),
                     exc_info=True)
...
def _signal_text(signals):
    return candidate_formatters._signal_text(signals, labels=SIGNAL_LABELS)
...
def _phase_d_lps_display_value(value, unknown="未知"):
    return candidate_formatters._phase_d_lps_display_value(value, unknown)


def _phase_d_lps_evidence_text(row):
    return candidate_formatters._phase_d_lps_evidence_text(
        row, display_value=_phase_d_lps_display_value)


def _phase_d_lps_shadow_markdown(shadow):
    return candidate_formatters._phase_d_lps_shadow_markdown(
        shadow, evidence_text=_phase_d_lps_evidence_text)


def _phase_d_lps_shadow_html(shadow):
    return candidate_formatters._phase_d_lps_shadow_html(
        shadow, evidence_text=_phase_d_lps_evidence_text)
...
def main():
    ...
    if news_file_path:
        try:
            ...
            LOGGER.debug("News fixture unavailable: %s", type(exc).__name__)
            news_evidence = {}
        ...
        try:
            ...
            LOGGER.debug("Live news fetch failed: %s", classify_failure(exc),
                         exc_info=True)
            news_evidence = {}
            news_fetch = {"status": "failed", "reason": type(exc).__name__}
    else:
