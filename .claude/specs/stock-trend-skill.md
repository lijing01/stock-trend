...
- 风险提示列表
- 操作建议区间（支撑/压力位）
- K线图表（仅 HTML 报告，含蜡烛图+成交量+MA均线）
- K线形态（识别出的形态列表）
- 特殊标记（港股/ETF/ST 等）

Markdown 报告中包含 HTML 报告链接：`> 📊 K线图表见: {timestamp}.html`
...
| HTML 报告 | 包含交互式 K 线图表（蜡烛图+成交量+MA 均线） |
...
HTML 报告中嵌入的交互式 K 线图表规格：

| 项目 | 规格 |
|---|---|
| 图表库 | ECharts 5.x（CDN 加载） |
| 图表类型 | 蜡烛图 + 成交量柱状图 + MA 均线叠加 |
| 均线周期 | MA5（黄）、MA10（蓝）、MA20（红）、MA60（灰） |
| 交互功能 | 悬浮提示（OHLCV+均线值）、缩放、拖拽 |
| 数据范围 | 最近 120 个交易日（可配置） |
| 配色方案 | 红涨绿跌（中国市场惯例） |
| 支撑/压力位 | 从技术分析结果中提取，以虚线标示在蜡烛图上 |
| 离线降级 | 无网络时图表区显示"图表加载需要网络连接"提示 |
| 图表生成脚本 | `scripts/generate_chart_html.py` |
| `--no-data` 模式 | 跳过图表生成，HTML 报告图表区隐藏 |
...
| 图表生成脚本失败 | — | HTML 报告跳过图表区段，MD 报告不含图表链接，其余内容正常生成 |
