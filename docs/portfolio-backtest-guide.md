...
| 模块 | 触发命令 | 用途 |
|------|----------|------|
| 持仓管理 | `/portfolio` | 记录持仓、跟踪盈亏、止损预警、与 etf-scan 排名对比 |
...
`status` = `list` + `alerts` + scan 对比。scan 对比会运行 `etf_scanner.py --output compact` 获取全量排名，然后对比持仓在排名中的位置：
...
etf-scan 的 Phase 1 速评分模型由 5 个维度组成（momentum 30%、volume 20%、capital_flow 20%、shares_trend 15%、iopv 15%），但只有 **momentum 和 volume** 有完整历史数据可回测。capital_flow、shares_trend、iopv 无历史数据，评分时为 None，权重自动重新分配。
...
1. **数据限制**：回测仅覆盖 momentum + volume 两个维度（纯 K 线派生），capital_flow/shares_trend/iopv 无历史数据。实际 etf-scan 使用全部 5 个维度时效果可能优于回测结果
