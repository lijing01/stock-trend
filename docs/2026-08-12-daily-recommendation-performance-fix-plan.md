...
- 正常网络、暖缓存：默认 `/candidates` 目标耗时不超过 60 秒（2026-08-13 经确认放宽）。
- 正常网络、冷缓存：按候选规模记录基线，优先确保无重复请求，并将耗时控制在可解释范围内。
- 数据源完全不可用：在 60 秒内完成缓存降级并返回，不再等待数分钟。
- 单次执行中，全市场板块排行最多获取一次。
...
- P0-2：跨批次按股票代码增量去重，同一股票只进入一次深度分析，保留完整板块归属，并用确定性比较键重绑主板块及派生评分。
- P0-3：K 线和维科夫漏斗前置，仅对通过者获取资金和基本面。
- P1-1：板块排行、成分、K 线、资金和基本面使用线程安全的运行级健康状态、有界探测、绝对截止时间和可审计熔断。
- P1-2：K 线、资金和基本面父进程缓存使用统一质量校验；无效缓存仅保留诊断，不能成为可执行推荐。交易日历有 2 秒上限、严格缓存 schema 校验，休市日与陈旧快照不会混淆。
- P1-3：JSON、stderr、Markdown、HTML 统一输出阶段耗时、漏斗、来源请求/缓存/失败/熔断和失败分类；输出装配只计时一次并冻结同一审计快照。

本次真实环境基准（命令：`daily_candidates.py --json --top 30 --min-candidates 20`）：

| 场景 | 总耗时 | 规模/漏斗 | 关键证据 |
|---|---:|---|---|
| 正常网络，首次运行（冷/部分缓存） | 40.21s | 5 批；109 原始、89 去重、4 维科夫通过 | K 线 23.131s、资金 14.078s；K 线 2 次超时后熔断 |
| 正常网络，后续暖缓存 | 40.10s | 5 批；165 原始、117 去重、10 维科夫通过 | K 线缓存命中 80；资金/基本面各命中 4；仍受新增实时数据和超时影响 |
| 数据源不可用（沙箱网络受限） | 16.69s | 5 批；165 原始、117 去重、10 维科夫通过 | 成分股仅 3 次实时请求后熔断；K 线仅 2 次后熔断；117 次 K 线缓存命中 |

所有场景均低于当前确认的 60 秒目标。三次运行均输出 0 个有效/可执行候选，说明提速没有绕过数据新鲜度和推荐资格门控。正常网络暖缓存仍未达到原 30 秒目标，因此保留性能基线并不把它误报为达标。

P2 去子进程化暂缓：当前 60 秒验收已通过，且审计证据首先指向网络实时请求/超时，而非仅由 Python 子进程启动成本主导。只有未来在稳定网络下暖缓存仍无法满足新的目标、并由阶段指标证明启动成本为主要瓶颈时，才另立 P2 ADR。
